...
public:
    ...
    {
        auto& storage = get_storage<DTO>();
        std::erase_if(storage, [id](const DTO& item) 
        {
            if constexpr (requires { item.id; }) 
            {
                return item.id == id;
...
TEST_F(SqlgenExampleTest, InsertMany_ShouldPersistMultiplePersons) 
{
    ...
    std::vector<Core::Person> newPeople{
        Core::Person(10, "batch1", "last1"),
...
TEST_F(SqlgenExampleTest, DependencyInjection_InterfacePolymorphism) 
{
    ...
    std::unique_ptr<Core::IPersonRepository> repo =
        std::make_unique<DAL::Repositories::SQLitePersonRepo<TestDatabaseConnection>>(conn);
